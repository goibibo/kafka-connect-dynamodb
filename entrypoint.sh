...
then
    ...
    exec /opt/kafka_2.11-${KAFKA_VERSION}/bin/connect-standalone.sh ./connect-properties/connect-standalone.properties ./connect-properties/sink.properties >> /logs/kafka-connect-dynamodb.log 2>&1
else
    /bin/bash -c "$*"
fi
